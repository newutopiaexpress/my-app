/**
 * This code was generated by v0 by Vercel.
 * @see https://v0.dev/t/bgW8MLC9pAQ
 */
import { Button } from "@/components/ui/button";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import Image from "next/image";
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from "@/components/ui/dialog";
import { Badge } from "@/components/ui/badge";
import Link from "next/link";
import { ThreeSteps } from "./three-steps";
import { CheckIcon} from '@radix-ui/react-icons';
import {
    Carousel,
    CarouselContent,
    CarouselItem,
    CarouselNext,
    CarouselPrevious,
  } from "@/components/ui/carousel";
  import Autoplay from "embla-carousel-autoplay";
  import { Separator } from "@/components/ui/separator";
  import Typewriter from 'typewriter-effect';
  

export function Launch() {
  return (

    

    <div id="launch" className="pb-6">


        <Separator className="w-full h-px bg-stone-300 mt-4 mb-4"/>
          <Link href="https://search.utopia.express" className="" target="_blank">
                <p className="text-xs uppercase tracking-wider text-stone-400 hover:text-stone-800">
                    <Badge></Badge>Rentable, customized AI services
                    <span className="float-right">Fill the form for details <ArrowIcon/></span>
                </p>
          </Link>
        <Separator className="w-full h-px bg-stone-300 mt-4 mb-4"/>


        <div className="md:col-span-1  content-center p-4 lg:p-9 mx-auto  border-stone-800 rounded-3xl relative">
            <h1 className="animate-fade bg-gradient-to-br from-stone-700 to-stone-800 bg-clip-text text-left  font-extrabold tracking-tighter text-transparent drop-shadow-sm [text-wrap:balance] text-5xl md:text-7xl md:leading-[6rem]">
              Are you a 
              <Typewriter
                    options={{
                    loop: true,
                    delay: 50,
                    }}
                    onInit={(typewriter) => {
                    typewriter
                        .typeString('stylist, or hairdresser?')
                        .pauseFor(1000)
                        .deleteAll()
                        .typeString('interior designer')
                        .pauseFor(1000)
                        .deleteAll()
                        .start();
                    }}
              />
            </h1>
            <p className="pt-4 text-xl font-normal text-stone-400 text-left">You bring the idea, we bring the tech. Start your own paid service or offer a futuristic tool to your customers</p>
        </div>

        <div className="grid md:grid-cols-12 md:gap-9 md:content-center md:p-9">


          <div className="md:col-span-8 content-center ">
            <Carousel 
                className="bg-transparent "
                plugins={[
                    Autoplay({
                      delay: 4000,
                    }),
                  ]}
                >
                <CarouselContent>
                    <CarouselItem className="relative">

                        <div className="absolute w-32 h-32 -top-6 left-6 z-10">
                                <Image
                                    alt="Image"
                                    className="float-left cover rounded-tl-full"
                                    src="/room11.jpg"
                                    width= "320"
                                    height= "262"
                                    style={{
                                    objectFit: "cover",
                                    }}
                                />
                          </div>

                        <div className="transition-all mx-auto grid grid-cols-2 gap-4">

                          <div className="col-span-1">
                              <Image
                                  alt="Image"
                                  className="cover rounded-2xl"
                                  src="/in.jpeg"
                                  width= "2048"
                                  height= "1152"
                                  style={{
                                  objectFit: "cover",
                                  }}
                              /> 
                          </div>

                          <div className="col-span-1">
                              <span className="w-32 h-32 text-stone-400 text-4xl">
                                    <DimensionsIcon/> <h1 className="py-4 text-2xl leading-5 font-bold text-stone-100">For interior designers & real estate agents</h1>
                              </span>
                              
                              <p className="leading-4 text-md text-purple-200">Create interior design variations based on a sketch or photo.</p>
                                
                          </div>

                        </div>
                    </CarouselItem>
                    <CarouselItem>
                        <div className="bg-stone-100/0 hover:bg-slate-900/20 transition-all p-6 rounded-2xl shadow-md shadow-slate-800/20 hover:shadow-slate-100/20">
                                <span className="text-stone-100 text-xl">
                                    <ScissorsIcon/>
                                </span>
                                <h1 className="py-4 text-xl leading-5 font-bold text-stone-100">For hairdressers & stylists</h1>
                                <p className="leading-4 text-sm text-slate-300">Show your customers some cool hairstyles before a haircut.</p>
                        </div>
                    </CarouselItem>
                    <CarouselItem>
                        <div className="bg-stone-100/0 hover:bg-slate-900/20 transition-all p-6 rounded-2xl shadow-md shadow-slate-800/20 hover:shadow-slate-100/20">
                                <span className="text-stone-100 text-xl">
                                    <RocketIcon/>
                                </span>
                                <h1 className="py-4 text-xl leading-5 font-bold text-stone-100">For webshops & product owners</h1>
                                <p className="leading-4 text-sm text-slate-300">Create virtual product photos in any style and enviroment.</p>
                        </div>
                    </CarouselItem>
                    <CarouselItem>
                        <div className="bg-stone-100/0 hover:bg-slate-900/20 transition-all p-6 rounded-2xl shadow-md shadow-slate-800/20 hover:shadow-slate-100/20">
                                <span className="text-stone-100 text-xl">
                                    <HeartIcon/>
                                </span>
                                <h1 className="py-4 text-xl leading-5 font-bold text-stone-100">Virtual try-on for clothes stores, designers</h1>
                                <p className="leading-4 text-sm text-slate-300">Enable your customers to try on clothes virtually.</p>
                        </div>
                    </CarouselItem>
                    <CarouselItem>
                        <div className="bg-stone-100/0 hover:bg-slate-900/20 transition-all p-6 rounded-2xl shadow-md shadow-slate-800/20 hover:shadow-slate-100/20">
                                <span className="text-stone-100 text-xl">
                                    <PlusIcon/>
                                </span>
                                <h1 className="py-4 text-xl leading-5 font-bold text-stone-100">Other use case?</h1>
                                <p className="leading-4 text-sm text-slate-300">Everything is possible. Drop us a message and we'll get in touch with you with a personalized offer!</p>
                        </div>
                    </CarouselItem>

                </CarouselContent>
                <CarouselPrevious/>
                <CarouselNext />
            </Carousel>
          </div>
          
          <div className="md:col-span-4">
            <h1 className="text-6xl font-extrabold tracking-tighter">Use cases</h1>
          </div>
            
        </div>

    </div>




  )
}


function PlusIcon() {
  return (
<svg className="w-6 h-6 opacity-50" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" >
  <path fillRule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25ZM12.75 9a.75.75 0 0 0-1.5 0v2.25H9a.75.75 0 0 0 0 1.5h2.25V15a.75.75 0 0 0 1.5 0v-2.25H15a.75.75 0 0 0 0-1.5h-2.25V9Z" clipRule="evenodd" />
</svg>
  )
}



  function ScissorsIcon() {
    return (
<svg className="w-6 h-6 opacity-50" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M0.949988 4.48455C0.949988 5.34059 1.64395 6.03455 2.49999 6.03455C3.35603 6.03455 4.04999 5.34059 4.04999 4.48455C4.04999 3.6285 3.35603 2.93455 2.49999 2.93455C1.64395 2.93455 0.949988 3.6285 0.949988 4.48455ZM2.49999 6.93455C1.14689 6.93455 0.0499878 5.83764 0.0499878 4.48455C0.0499878 3.13145 1.14689 2.03455 2.49999 2.03455C3.85309 2.03455 4.94999 3.13145 4.94999 4.48455C4.94999 4.80813 4.88726 5.11707 4.77329 5.39985L5.73655 6.04413C5.72943 6.06656 5.72269 6.08914 5.71635 6.11188L5.56288 6.66186L5.21005 6.89773L4.21554 6.23366C3.77357 6.6672 3.168 6.93455 2.49999 6.93455ZM0.949991 10.5C0.949991 9.64394 1.64395 8.94998 2.49999 8.94998C3.35603 8.94998 4.04999 9.64394 4.04999 10.5C4.04999 11.356 3.35603 12.05 2.49999 12.05C1.64395 12.05 0.949991 11.356 0.949991 10.5ZM2.49999 8.04998C1.14689 8.04998 0.0499908 9.14688 0.0499908 10.5C0.0499908 11.8531 1.14689 12.95 2.49999 12.95C3.85309 12.95 4.94999 11.8531 4.94999 10.5C4.94999 10.1803 4.88877 9.87497 4.77743 9.595L15 2.75737L14.0486 2.85737C12.7077 2.9983 11.4091 3.40895 10.231 4.06461L7.15646 5.77564C6.92307 5.90553 6.75134 6.12339 6.67955 6.38066L6.42653 7.28737L4.22437 8.75957C3.78166 8.32091 3.17246 8.04998 2.49999 8.04998ZM7.14435 9.2149L7.15646 9.22176L10.231 10.9328C11.4091 11.5884 12.7077 11.9991 14.0486 12.14L15 12.24L8.81072 8.1003L7.14435 9.2149Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
    )
  }

  function DimensionsIcon() {
    return (
      <svg className="w-14 h-14 opacity-100 float-left" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.00014 2.73895C3.00014 2.94698 2.76087 3.06401 2.59666 2.93628L1.00386 1.69744C0.875177 1.59735 0.875177 1.40286 1.00386 1.30277L2.59666 0.063928C2.76087 -0.0637944 3.00014 0.0532293 3.00014 0.261266V1.00012H9.00009V0.261296C9.00009 0.0532591 9.23936 -0.0637646 9.40358 0.0639578L10.9964 1.3028C11.1251 1.40289 11.1251 1.59738 10.9964 1.69747L9.40358 2.93631C9.23936 3.06404 9.00009 2.94701 9.00009 2.73898V2.00012H3.00014V2.73895ZM9.50002 4.99998H2.50002C2.22388 4.99998 2.00002 5.22384 2.00002 5.49998V12.5C2.00002 12.7761 2.22388 13 2.50002 13H9.50002C9.77616 13 10 12.7761 10 12.5V5.49998C10 5.22384 9.77616 4.99998 9.50002 4.99998ZM2.50002 3.99998C1.67159 3.99998 1.00002 4.67156 1.00002 5.49998V12.5C1.00002 13.3284 1.67159 14 2.50002 14H9.50002C10.3284 14 11 13.3284 11 12.5V5.49998C11 4.67156 10.3284 3.99998 9.50002 3.99998H2.50002ZM14.7389 6.00001H14V12H14.7389C14.9469 12 15.064 12.2393 14.9362 12.4035L13.6974 13.9963C13.5973 14.125 13.4028 14.125 13.3027 13.9963L12.0639 12.4035C11.9362 12.2393 12.0532 12 12.2612 12H13V6.00001H12.2612C12.0532 6.00001 11.9361 5.76074 12.0639 5.59653L13.3027 4.00373C13.4028 3.87505 13.5973 3.87505 13.6974 4.00374L14.9362 5.59653C15.0639 5.76074 14.9469 6.00001 14.7389 6.00001Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
    )
  }

  function RocketIcon() {
    return (
      <svg className="w-6 h-6 opacity-50" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.85357 3.85355L7.65355 3.05353C8.2981 2.40901 9.42858 1.96172 10.552 1.80125C11.1056 1.72217 11.6291 1.71725 12.0564 1.78124C12.4987 1.84748 12.7698 1.97696 12.8965 2.10357C13.0231 2.23018 13.1526 2.50125 13.2188 2.94357C13.2828 3.37086 13.2779 3.89439 13.1988 4.44801C13.0383 5.57139 12.591 6.70188 11.9464 7.34645L7.49999 11.7929L6.35354 10.6465C6.15827 10.4512 5.84169 10.4512 5.64643 10.6465C5.45117 10.8417 5.45117 11.1583 5.64643 11.3536L7.14644 12.8536C7.34171 13.0488 7.65829 13.0488 7.85355 12.8536L8.40073 12.3064L9.57124 14.2572C9.65046 14.3893 9.78608 14.4774 9.9389 14.4963C10.0917 14.5151 10.2447 14.4624 10.3535 14.3536L12.3535 12.3536C12.4648 12.2423 12.5172 12.0851 12.495 11.9293L12.0303 8.67679L12.6536 8.05355C13.509 7.19808 14.0117 5.82855 14.1887 4.58943C14.2784 3.9618 14.2891 3.33847 14.2078 2.79546C14.1287 2.26748 13.9519 1.74482 13.6035 1.39645C13.2552 1.04809 12.7325 0.871332 12.2045 0.792264C11.6615 0.710945 11.0382 0.721644 10.4105 0.8113C9.17143 0.988306 7.80189 1.491 6.94644 2.34642L6.32322 2.96968L3.07071 2.50504C2.91492 2.48278 2.75773 2.53517 2.64645 2.64646L0.646451 4.64645C0.537579 4.75533 0.484938 4.90829 0.50375 5.0611C0.522563 5.21391 0.61073 5.34954 0.742757 5.42876L2.69364 6.59928L2.14646 7.14645C2.0527 7.24022 2.00002 7.3674 2.00002 7.50001C2.00002 7.63261 2.0527 7.75979 2.14646 7.85356L3.64647 9.35356C3.84173 9.54883 4.15831 9.54883 4.35357 9.35356C4.54884 9.1583 4.54884 8.84172 4.35357 8.64646L3.20712 7.50001L3.85357 6.85356L6.85357 3.85355ZM10.0993 13.1936L9.12959 11.5775L11.1464 9.56067L11.4697 11.8232L10.0993 13.1936ZM3.42251 5.87041L5.43935 3.85356L3.17678 3.53034L1.80638 4.90074L3.42251 5.87041ZM2.35356 10.3535C2.54882 10.1583 2.54882 9.8417 2.35356 9.64644C2.1583 9.45118 1.84171 9.45118 1.64645 9.64644L0.646451 10.6464C0.451188 10.8417 0.451188 11.1583 0.646451 11.3535C0.841713 11.5488 1.1583 11.5488 1.35356 11.3535L2.35356 10.3535ZM3.85358 11.8536C4.04884 11.6583 4.04885 11.3417 3.85359 11.1465C3.65833 10.9512 3.34175 10.9512 3.14648 11.1465L1.14645 13.1464C0.95119 13.3417 0.951187 13.6583 1.14645 13.8535C1.34171 14.0488 1.65829 14.0488 1.85355 13.8536L3.85358 11.8536ZM5.35356 13.3535C5.54882 13.1583 5.54882 12.8417 5.35356 12.6464C5.1583 12.4512 4.84171 12.4512 4.64645 12.6464L3.64645 13.6464C3.45119 13.8417 3.45119 14.1583 3.64645 14.3535C3.84171 14.5488 4.1583 14.5488 4.35356 14.3535L5.35356 13.3535ZM9.49997 6.74881C10.1897 6.74881 10.7488 6.1897 10.7488 5.5C10.7488 4.8103 10.1897 4.25118 9.49997 4.25118C8.81026 4.25118 8.25115 4.8103 8.25115 5.5C8.25115 6.1897 8.81026 6.74881 9.49997 6.74881Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
    )
  }

  function HeartIcon() {
    return (
      <svg className="w-6 h-6 opacity-50" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M4.89346 2.35248C3.49195 2.35248 2.35248 3.49359 2.35248 4.90532C2.35248 6.38164 3.20954 7.9168 4.37255 9.33522C5.39396 10.581 6.59464 11.6702 7.50002 12.4778C8.4054 11.6702 9.60608 10.581 10.6275 9.33522C11.7905 7.9168 12.6476 6.38164 12.6476 4.90532C12.6476 3.49359 11.5081 2.35248 10.1066 2.35248C9.27059 2.35248 8.81894 2.64323 8.5397 2.95843C8.27877 3.25295 8.14623 3.58566 8.02501 3.88993C8.00391 3.9429 7.98315 3.99501 7.96211 4.04591C7.88482 4.23294 7.7024 4.35494 7.50002 4.35494C7.29765 4.35494 7.11523 4.23295 7.03793 4.04592C7.01689 3.99501 6.99612 3.94289 6.97502 3.8899C6.8538 3.58564 6.72126 3.25294 6.46034 2.95843C6.18109 2.64323 5.72945 2.35248 4.89346 2.35248ZM1.35248 4.90532C1.35248 2.94498 2.936 1.35248 4.89346 1.35248C6.0084 1.35248 6.73504 1.76049 7.20884 2.2953C7.32062 2.42147 7.41686 2.55382 7.50002 2.68545C7.58318 2.55382 7.67941 2.42147 7.79119 2.2953C8.265 1.76049 8.99164 1.35248 10.1066 1.35248C12.064 1.35248 13.6476 2.94498 13.6476 4.90532C13.6476 6.74041 12.6013 8.50508 11.4008 9.96927C10.2636 11.3562 8.92194 12.5508 8.00601 13.3664C7.94645 13.4194 7.88869 13.4709 7.83291 13.5206C7.64324 13.6899 7.3568 13.6899 7.16713 13.5206C7.11135 13.4709 7.05359 13.4194 6.99403 13.3664C6.0781 12.5508 4.73641 11.3562 3.59926 9.96927C2.39872 8.50508 1.35248 6.74041 1.35248 4.90532Z" fill="currentColor" fill-rule="evenodd" clip-rule="evenodd"></path></svg>
    )
  }

  function ArrowIcon() {
    return (
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1} stroke="currentColor" className="float-right ml-2 w-4 h-4">
    <path strokeLinecap="round" strokeLinejoin="round" d="m4.5 19.5 15-15m0 0H8.25m11.25 0v11.25" />
  </svg>
    )
  }
  


